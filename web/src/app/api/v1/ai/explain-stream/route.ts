/**
 * Proxy for POST /api/v1/ai/explain-stream → backend /api/v1/ai/explain-stream.
...
export async function POST(req: NextRequest) {
  const url = `${BACKEND}/api/v1/ai/explain-stream`;
  ...
  try {
    ...
    // Backend returned a non-SSE response — should be JSON with task_id.
    ...
    console.error(
      `[explain-stream-proxy] POST ${url} failed: ${isTimeout ? "timeout" : "error"} — ${msg}`
    );
